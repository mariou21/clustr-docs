...
- Descripción opcional por habitación: se hereda automáticamente del tipo precargado del proveedor, es editable y plegable por habitación, y se imprime en el PDF
...
- Asociació con proveedor específico
...
- Datos de contacto: correo, teléfono, direcció completa
- Informació de pasaporte y fecha de nacimiento
- Estado de Visa USA con fecha de vencimiento
- Canal de procedencia (WhatsApp, Instagram, etc.)
- Preferencia de suscripció informativa
...
Directorio de proveedores de servicios turísticos con categorizació y contactos.

**Características:**

- ID único personalizable por proveedor
- Categorías configurables (Hotel, Transporte, Tour Operador, etc.)
- Información de contacto completa (persona, teléfono, correo, direcció)
- Asignació de múltiples destinos operativos
- Logo/imagen del proveedor (máx. 1MB)
- Inclusiones y servicios ofrecidos
- Tipos de habitación precargados por alojamiento (cama, vista, dormitorios, capacidades y descripció opcional en texto plano)
- Editor de habitación en drawer amplio de dos columnas, con indicador de descripció en la tabla de tipos configurados
...
- Creació de itinerarios reutilizables con estructura modular
- Soporte para múltiples trayectos con conexiones y escalas
- Configuració de tiempo de conexión (horas/minutos)
- Generació automática de códigos de itinerario (ORIG-DEST-SECUENCIAL)
...
- Botó "Cotizar" para abrir inmediatamente una cotizació con el itinerario
- Acciones: crear, editar, duplicar, eliminar, usar en cotizació
- Validació automática de campos obligatorios por trayecto

## Módulo de Configuració

Panel centralizado para configurar todos los aspectos operativos y visuales del sistema.

**Secciones:**

1. **General** - Empresa, contacto, moneda, zona horaria, papelera, oficinas y versió del sistema.
2. **Usuarios** - Gestió de usuarios y grupos con permisos granulares por módulo.
...
9. **Canales** - Canales de captació del cliente.
10. **Plantillas** - Correos, vista pública, mensajería y diseñador PDF.
11. **Documentació** - Portal técnico interno actualizado desde el sistema.
12. **Papelera** - Restauració o eliminació definitiva según retención configurada.
